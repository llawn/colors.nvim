...
local palette_file = conf.get("palette.file_path")
...
local function save_palettes()
	...
	for name, palette_data in pairs(M.saved_palettes) do
		...
		for _, color in ipairs(palette_data.colors) do
			table.insert(lines, string.format("      0x%06x,", color))
		end
		table.insert(lines, "    },")
...
--- @param palette table Array of color integers
...
--- @param old_name string Current palette name
--- @param new_name string New palette name
--- @return boolean: Success status
function M.rename_palette(old_name, new_name)
	if not M.saved_palettes[old_name] then
		return false
	end

	if M.saved_palettes[new_name] then
		return false
	end

	M.saved_palettes[new_name] = M.saved_palettes[old_name]
	M.saved_palettes[old_name] = nil
	save_palettes()
	return true
end
...
function M.export_palette_to_lua(palette, name)
	local code = string.format("-- Palette: %s\nlocal %s = {\n", name, name:gsub("[^%w_]", "_"))

	for i, color in ipairs(palette) do
		code = code .. string.format("  0x%06x", color)
		if i < #palette then
			code = code .. ","
		end
		code = code .. "\n"
...
function M.import_palette_from_lua(lua_code, name)
	...
	for line in lua_code:gmatch("[^\r\n]+") do
		local color_match = line:match("(0x%x+)")
		if color_match then
			table.insert(colors, tonumber(color_match))
		end
	end
...
function M.get_palette_stats()
	...
	for _, palette_data in pairs(M.saved_palettes) do
